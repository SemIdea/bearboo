...
- **Generated column + GIN index.** A `searchVector tsvector GENERATED ALWAYS AS (to_tsvector('portuguese', title || ' ' || content)) STORED` column, kept in sync by Postgres, with a GIN index. Created in a hand-written migration (Prisma cannot express `GENERATED ALWAYS AS`); declared `Unsupported("tsvector")?` in the schema so Prisma tracks it and does not drop it on drift.
- **`portuguese` dictionary.** Matches the blog's pt-BR content (stems `programação`↔`programar`, drops pt stop-words). English/code terms still match as tokens.
