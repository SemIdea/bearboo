...
- `src/context/trpc/fetcher.ts` (`customFetcher`) reimplementa na mão o parsing do envelope de erro do `httpBatchLink` (array + `error.json.message` do superjson) pra decidir se refaz a sessão. **2026-07-11: corrigido um bug nesse parsing** (lia `body.error.code`, formato que não existe no batch — sessão nunca era renovada, só falhava com 401 repetido; ver `extractSessionErrorCode` + `src/context/trpc/__test__/fetcher.ts`), mas o desenho em si continua frágil: qualquer mudança no formato interno de serialização do tRPC quebra esse parsing de novo, silenciosamente. Melhor caminho: mover essa lógica pra um link customizado do tRPC (`opts.next(op)`), que recebe o erro já tipado (`TRPCClientError`) em vez de JSON cru. Não fechado — falta decidir se entra junto com o hardening de `docs/features/001-auth-hardening/spec.md` (que já cobre o problema relacionado de accessToken/refreshToken legíveis por JS via `document.cookie`/`localStorage` em vez de cookie `HttpOnly`) ou como item avulso.
- `/post/[slug]` (`docs/features/002-post-slug/`) agora chama `notFound()` corretamente quando o slug não existe (**2026-07-11**, antes caía no `error.tsx` genérico — ver commit `a620cde`), mas o status HTTP da resposta continua `200`, não `404`. Causa: `PostContent` roda dentro de um `<Suspense>` (streaming) — o shell da resposta já é enviado com 200 antes do `notFound()` disparar dentro do boundary, então o status não pode mais mudar. Corrigir de verdade exige tirar a leitura do post de dentro do `<Suspense>` (perde o fallback "Loading post..."), o que é mudança de padrão de streaming, não fix pontual. Relevante pra Fase 5 (SEO) do roadmap — bots que checam status HTTP não veem 404 real.
