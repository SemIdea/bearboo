...
- **`DomainError` genérico — primeiro uso da regra 15 forward-only:** `src/shared/error/domainError.ts` (`class DomainError<C extends string>`), desenho já prescrito em `docs/rubrics/error-classification.md` (Opção B) mas nunca usado até aqui — todo domain existente lança `TRPCError` direto (débito rastreado em `afm.md § 3.1`). `media/domain/{upload,readOwn,delete}.ts` não importam `TRPCError`; a procedure de delete (`media/procedures/delete.ts`) mapeia `DomainError` → `TRPCError` no boundary (`NOT_FOUND`/`FORBIDDEN`), único ponto que conhece transport.
...
#### Shared error — ErrorRegistry (`ADR-0017`, migração fechada em `022-error-registry`)

`src/shared/error/registry.ts` — `defineDomainErrors(domain, errors)` namespaces cada catálogo por domínio (`"auth.invalid_credentials"` etc.) e valida contra domínio duplicado em runtime (`Set`). `src/shared/error/index.ts` agrega os 8 catálogos (`auth`, `comment`, `media`, `post`, `resetToken`, `session`, `user`, `verifyToken`) num `Errors`/`ErrorCode` central. `DomainError` (`domainError.ts`) recebe só o `code` namespaced e resolve `httpCode`/`message` sozinho — nenhuma procedure escreve switch/if-chain pra traduzir erro de domínio, só o bloco genérico `if (error instanceof DomainError) throw new TRPCError({code: error.httpCode, message: error.message, cause: error})`.

**Regra dura 15 fechada:** `rg -l "TRPCError" src/server/features/*/domain/*.ts` retorna vazio — os 23 arquivos que lançavam `TRPCError` direto (contagem no início de `022-error-registry`) migraram pra `DomainError`. Não é mais forward-only (removido de `afm.md` § 3.1).

**Catálogos aditivos vs. cutover:** `comment`/`media`/`resetToken`/`user`/`verifyToken` tiveram o enum antigo (`<Feature>ErrorCode`/`<Feature>ErrorMessages`) totalmente removido — sem consumidor fora do próprio domain/procedure/teste. `auth`/`session`/`post` **mantêm o enum antigo lado a lado** com o catálogo novo, porque têm consumidor legítimo fora da camada de domínio: `createRouter.ts` (guards `publicProcedure`/`protectedProcedure`/`verifiedProcedure`/`roleProcedure` lançam `TRPCError` com o enum direto — camada correta, não é violação), `src/server/caller.ts`, `src/context/trpc/sessionRefreshLink.ts` (client-side, decide refresh/redirect por código de erro) e `src/app/(half)/post/[slug]/page.tsx` + `analytics/procedures/recordView.ts`. Enum antigo não é shim morto (regra dura 10) — os callers genuinamente continuam existindo.

**Dois jeitos de ler `error.cause` no boundary:** `createCaller()` (server-side, in-process — `post/[slug]/page.tsx`) lê `error.cause instanceof DomainError` direto, sem round-trip de rede. Client-side sobre HTTP (`sessionRefreshLink.ts`) não tem acesso a `.cause` do objeto original — o `errorFormatter` de `createRouter.ts` propaga o `code` namespaced via `data.domainCode` (campo próprio, não confundir com `data.code` nativo do tRPC que já é o HTTP code).

`src/lib/error.ts` (`getErrorMessage`) simplificou pra um único lookup com pass-through: resolve code legado quando reconhece (`allErrorMessages[code]`), senão assume que já é texto final (migrado) e devolve como está — os ~13 call-sites do frontend não precisaram mudar.
