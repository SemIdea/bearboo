...
| 10 — CI/CD and deploy | 🟡 Partial | CI pipeline exists (`.github/workflows/ci.yml`: `typecheck`/`lint`/`test`/`build` on Node 20, on PR + push to `develop`/`main`) and PRs deploy via Vercel; missing: infra hardening — TLS/HTTPS via nginx + certificate (the `docker-compose.yml` hardcoded credentials + exposed datastore ports were fixed 2026-08-29 — secrets via env, no host ports for Postgres/Redis in prod), README run instructions, a confirmed stable production environment |
...
>
> **Resolved (`027`, 2026-08-26):** the integration harness landed in feature 026 (Testcontainers + real Postgres, ADR-0026), so native full-text shipped in `docs/features/027-native-fulltext-search/` (ADR-0027): a generated `tsvector` column + GIN index, `ts_rank` relevance ordering (default when a query is present), `english` stemming, consistent matching across sorts. The search tests moved to `*.integration.ts` (the mock cannot run tsvector).
...
* [x] validation with Zod (`src/server/features/*/schema.ts`);
...
**Item inherited from `001-auth-hardening` (2026-07-12) — addressed 2026-08-29:** the production `docker-compose.yml` had hardcoded Postgres credentials (`postgres`/`postgres`) and exposed the Postgres port to the host without need. **Fixed:** credentials now come from env (`POSTGRES_USER`/`POSTGRES_PASSWORD`/`POSTGRES_DB`, with `POSTGRES_PASSWORD` required — the compose fails to start without it and the app's `DATABASE_URL` is built from them), and the host port mappings for Postgres **and** Redis were removed (the app reaches them over the Docker network `postgres:5432`/`cache:6379`). Still open in this phase: TLS/HTTPS via nginx + certificate (depends on a domain/certificate).
