...
| 9 — Production quality | 🟡 Partial | Zod, migrations, seed, some tests/error pages already exist; **structured logs done** (`docs/features/025-structured-logging/`, ADR-0022 — canonical log line per call, JSON prod / pretty dev); in-memory rate limiting covers login/register/reset/refresh (`docs/features/001-auth-hardening/`) — not general rate limiting for the whole API; missing: integration + e2e tests (backend unit coverage is already ~90%; frontend ~0%, deferred), general API rate limiting |
...
* [x] unit tests (85 `vitest` files, 411 tests — backend coverage ~90% via v8, measured 2026-08-25; whole-repo ~52% is dragged down by the untested frontend, deferred with the frontend refactor — see `docs/afm.md` § 3.1);
* [ ] integration tests;
* [ ] basic e2e tests;
