...
36. **The boundary emits one canonical log line per call; the logger never carries a secret.** Every procedure call emits exactly one wide structured event at the boundary (`withCanonicalLog`), success or failure — JSON in prod, pretty in dev (`env.nodeEnv`). Enrich a line with `ctx.log.add({ ... })`; `LogFields` is scalar-only, so a raw `input`/`ctx` object cannot be dumped, and a sensitive-key scrub (`token`/`password`/`secret`/`authorization`/`cookie`) drops such keys on emit (rule 13). No `console.*` in `src/server/**`, except the sanctioned mock mailer transport. Logs go to stdout as one event stream (12-Factor); shipping/aggregation is the environment's job. See ADR-0022.
