...
- [X] T005 — RED: teste `src/server/http/__test__/withDomainErrors.ts` — `DomainError` do resolver vira o código de transporte correto; throw não-`DomainError` continua `INTERNAL_SERVER_ERROR`; `cause` preservado (US-017)
