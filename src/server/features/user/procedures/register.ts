...
const procedure_registerUser = publicProcedure
	...
	.mutation(async ({ input, ctx }) => {
        ...
        <p><a href="${ctx.env.siteUrl}/auth/verify/${verifyToken.token}" style="background-color: #3B82F6; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; display: inline-block;">Verify Email Address</a></p>
        <p>Or copy and paste this link into your browser:</p>
        <p>${ctx.env.siteUrl}/auth/verify/${verifyToken.token}</p>
